...
export default function ContactDetailsPage() {
  ...
  const mapId: string | undefined = process.env
    .NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID as string | undefined;

  const [expandedContactIndex, setExpandedContactIndex] = useState<
    number | null
  >(null);
  const [applianceSearchTerm, setApplianceSearchTerm] = useState("");
  const [expandedApplianceIndex, setExpandedApplianceIndex] = useState<
    number | null
  >(null);
  ...
  const [activeApplianceTab, setActiveApplianceTab] = useState("service");

  const toggleApplianceAccordion = (index: number) => {
    if (expandedApplianceIndex !== index) {
      setActiveApplianceTab("service");
    }
    setExpandedApplianceIndex(expandedApplianceIndex === index ? null : index);
  };
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      ...
      try {
        ...
        const snapshot = await getDocs(q);
        const loadedCertificates = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  useEffect(() => {
    const loadRelated = async () => {
      ...
      try {
                ...
                if (data.engineerId) {
                  try {
                    const engSnap = await getDoc(
                      doc(db, "USERS", USER_ID, "engineers", data.engineerId),
                    );
                    if (engSnap.exists()) {
        ...
        past.sort(
          (a, b) =>
            new Date(b.start || a.date).getTime() -
  ...
  const geocodeIfNeeded = async (): Promise<{ lat: number; lng: number }> => {
    ...
    try {
      ...
      setContact((prev) =>
        prev ? ({ ...prev, addressLocation: latLng } as Contact) : prev,
      );
    } catch (e) {
      ...
      if (!mapInstanceRef.current) {
        ...
        if (mapId && google.maps.marker?.AdvancedMarkerElement) {
          ...
          if (!mapId) {
            ...
            console.info(
              "[Map] No NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID configured; using legacy Marker.",
            );
          }
          markerInstanceRef.current = new google.maps.Marker({
            position: latLng,
        ...
        sv.getPanorama(
          ...
          (data: any, status: string) => {
            if (status === "OK" && data) resolve(data);
              ...
              reject(
                new Error("No Street View imagery found near this address"),
              );
          },
        );
      });
      ...
      {isAuthenticated && !loading && contact && (
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border-2 border-gray-100 mb-6">
              ...
              {contact.disableCommunication && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-red-800">
                    Communication disabled with this customer
                  </span>
                </div>
              )}
            </div>
              ...
              {appliances.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Add an appliance to see it here
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {appliances
                    .filter(
                      (app) =>
                        app.location
                          .toLowerCase()
                          .includes(applianceSearchTerm.toLowerCase()) ||
                        app.type
                          .toLowerCase()
                          .includes(applianceSearchTerm.toLowerCase()) ||
                        app.manufacturer
                    ...
                    .map((appliance, index) => (
                      ...
                      >
                        <div
                          className="flex items-center justify-between p-3 bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors"
                          onClick={() => toggleApplianceAccordion(index)}
                        >
                          <div>
                            <h4 className="font-semibold text-gray-900 text-sm">
                          ...
                          <svg
                            className={`w-5 h-5 text-gray-400 transition-transform ${expandedApplianceIndex === index ? "rotate-180" : ""}`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                        ...
                        {expandedApplianceIndex === index && (
                          <div className="bg-white border-t border-gray-200 text-sm">
                            {/* Tabs */}
                            <div className="flex border-b border-gray-200 px-3">
                              <button
                                className={`py-3 px-4 font-semibold border-b-2 transition-colors ${activeApplianceTab === "service" ? "border-green-600 text-green-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setActiveApplianceTab("service");
                                }}
                              ...
                              <button
                                className={`py-3 px-4 font-semibold border-b-2 transition-colors ${activeApplianceTab === "info" ? "border-green-600 text-green-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setActiveApplianceTab("info");
                                }}
                              >
                                Appliance Info
                              </button>
                              <button
                                className={`py-3 px-4 font-semibold border-b-2 transition-colors ${activeApplianceTab === "technical" ? "border-green-600 text-green-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setActiveApplianceTab("technical");
                                }}
                              >
                                Technical Details
                              </button>
                            ...
                            <div className="p-4">
                                ...
                                <div className="space-y-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      ...
                                      <p className="text-gray-900">
                                        {appliance.serviceFrequency} Months
                                      </p>
                                    </div>
                                    <div>
                                      <p className="text-xs font-semibold text-gray-500 uppercase mb-1">
                                ...
                                <div className="space-y-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-gray-900">
                                        {appliance.installedBy || "-"}
                                      </p>
                                    </div>
                                    {appliance.linkedContactName && (
                                      <div>
                                        <p className="text-xs font-semibold text-gray-500 uppercase mb-1">
                                  ...
                                  {appliance.imageUrls &&
                                    appliance.imageUrls.length > 0 && (
                                      <div>
                                        <p className="text-xs font-semibold text-gray-500 uppercase mb-1">
                                          Photos
                                        </p>
                                        <div className="flex gap-2 overflow-x-auto pb-2">
                                          {appliance.imageUrls.map((url, i) => (
                                            <img
                                              key={i}
                                              src={url}
                                              alt={`Appliance ${i + 1}`}
                              ...
                              {activeApplianceTab === "technical" && (
                                <div className="space-y-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      <p className="text-xs font-semibold text-gray-500 uppercase mb-1">
                                        Default Sweeping Method
                                      </p>
                                      <p className="text-gray-900">
                                        {appliance.defaultSweepingMethod || "-"}
                                      </p>
                                    </div>
                                    <div>
                                      <p className="text-xs font-semibold text-gray-500 uppercase mb-1">
            ...
            <div className="mt-6 border-t pt-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    router.push(`/admin/contacts/${contactId}/certificates/new`)
                  }
              ...
              {certificates.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Issue a certificate to see it here
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {certificates
                    ...
                    .map((certificate, index) => (
                      <div
                        ...
                        onClick={() =>
                          router.push(
                            `/admin/contacts/${contactId}/certificates/${certificate.id}`,
                          )
                        }
                      >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs text-gray-600">
                              <div>
                                ...
                                {certificate.templateName || "N/A"}
                              </div>
                              <div>
                                <span className="font-medium">Appliance:</span>{" "}
                                {certificate.appliance?.location || "N/A"}
                              </div>
                              <div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border-2 border-gray-100">
            ...
            <div
              className={`mt-4 transition-opacity ${viewMode === "none" ? "opacity-0 h-0 overflow-hidden" : "opacity-100"} `}
            >
              <div
                className={`relative w-full ${viewMode === "none" ? "h-0" : "h-96"} rounded-lg overflow-hidden border border-gray-200`}
              >
                <div
                  ref={mapDivRef}
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border-2 border-gray-100 mt-6">
            ...
            {loadingAppointments ? (
              ...
              <div className="text-gray-600">
                No upcoming appointments for this contact.
              </div>
            ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-base font-medium text-gray-900">
                          {apt.start ? formatDisplayTime(apt.start) : "—"}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-gray-500">Engineer</p>
                        <p className="text-base font-semibold text-gray-900">
                          {apt.engineerName || apt.engineerId || "Not assigned"}
                        </p>
                      </div>
                    </div>
                  </Link>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border-2 border-gray-100 mt-6">
            ...
            {loadingAppointments ? (
              ...
              <div className="text-gray-600">
                No past appointments found for this contact.
              </div>
            ) : (
              <div className="divide-y">
