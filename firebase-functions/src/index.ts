...
export const getAvailableSlots = onCall<GetAvailableSlotsInput>(
  { region: "europe-west2" },
  async (request) => {
    ...
    if (distanceFromOfficeMiles > MAX_OFFICE_RADIUS_MILES) {
      return {
        ok: false,
        reason: `Location is ${distanceFromOfficeMiles.toFixed(1)} miles from office (max ${MAX_OFFICE_RADIUS_MILES} miles)`,
      };
    }
    ...
    for (const doc of daysSnap.docs) {
      ...
      const bookedAppts = appointmentsSnap.docs
        .map((d) => {
          const a = d.data();
          return {
            start: a.start,
            end: a.end,
            location: a.address?.location as
      ...
      if (bookedAppts.length === 0) {
        ...
        if (
          slotDate.getUTCHours() >= EARLIEST_TIME_HOUR &&
          slotDate.getUTCHours() < LATEST_TIME_HOUR
        ) {
      ...
      for (const appt of bookedAppts) {
        ...
        const proposedStart = new Date(
          apptEnd.getTime() + bufferMinutes * 60 * 1000,
        );
        const proposedEnd = new Date(
          proposedStart.getTime() + SLOT_DURATION_MINUTES * 60 * 1000,
        );

        // Check if proposed slot is within working hours
        const hourUTC = proposedStart.getUTCHours();
        if (hourUTC < EARLIEST_TIME_HOUR || hourUTC >= LATEST_TIME_HOUR)
          continue;
    ...
    daysSnap.docs.forEach((d) =>
      engineersSet.add((d.data() as any).engineerId),
    );
...
}> = [
  {
    ...
    slots: [
      ...
      },
      {
        ...
        end: "2025-11-02T11:30:00Z",
        booked: false,
      },
      {
        start: "2025-11-02T12:00:00Z",
        end: "2025-11-02T13:00:00Z",
        booked: false,
      },
    ],
  },
...
export const seedSampleData = onCall<SeedInput>(
  { region: "europe-west2" },
  async (request) => {
    ...
    for (const a of SAMPLE_APPOINTMENTS) {
      if (a.id) {
        ...
        await getUserCollection(userId, "appointments")
          .doc(String(id))
          .set(rest);
      } else {
...
export const bookAppointment = onCall<BookAppointmentInput>(
  { region: "europe-west2" },
  async (request) => {
    try {
      ...
      const monthStart = new Date(
        Date.UTC(yyyy as number, (mm as number) - 1, 1),
      );
      const monthEnd = new Date(
        Date.UTC(yyyy as number, mm as number, 0, 23, 59, 59),
      );
      ...
      const result = await db.runTransaction(async (tx) => {
        ...
        if (conflict) {
          return {
            ok: false,
            reason: "Slot just taken, please pick another",
          } as const;
        }
        ...
        if (!daySnap.exists) {
          ...
          const slots: any[] = Array.isArray(dayData.slots)
            ? [...dayData.slots]
            : [];
...
export * from "./generateCertificatePDF";
